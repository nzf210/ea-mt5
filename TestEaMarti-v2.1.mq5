...
void OnTick()
{
   ...
   if (jmlTrxTerbuka == 0) { 
      ...
      if(buyType == "BUY"){
         ...
         textDebug = "Jml trx terbuka: textDebugBuy" + jmlTrxTerbuka;
      }
      ...
      if(buyType == "SELL"){
         ...
         textDebug = "Jml trx terbuka: textDebugSell" + jmlTrxTerbuka;
      }

      // for (int i = 0; i < totalPositions; i++) {
      // if (PositionSelectByTicket(1)) {
      //       double positionProfit = PositionGetDouble(POSITION_PROFIT); // Profit/loss in the deposit currency
      //       double positionOpenPrice = PositionGetDouble(POSITION_PRICE_OPEN);
      //       double currentPrice = (_Symbol == POSITION_SYMBOL) ? SymbolInfoDouble(_Symbol, SYMBOL_BID) : 0.0;

      //       double pipValue = SymbolInfoDouble(_Symbol, SYMBOL_POINT); // Value of one pip
      //       double positionPips = (currentPrice - positionOpenPrice) / pipValue;

      //       // If the position is long (BUY)
      //       if (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY) {
      //           // Calculate profit/loss in pips for BUY positions
      //          double profitInPips = positionPips;
      //          if(profitInPips <= 20 && jmlTrxTerbuka == 1) {
      //             openPrice = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
      //             MarketOrder(ORDER_TYPE_BUY,jmlTrxTerbuka);
      //             jmlTrxTerbuka += 1 ;
      //          }
      //          if(profitInPips <= 40 && jmlTrxTerbuka == 2) {
      //             openPrice = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
      //             MarketOrder(ORDER_TYPE_BUY,jmlTrxTerbuka);
      //             jmlTrxTerbuka += 1 ;
      //          }
      //          Print("Profit/Loss in Pips for BUY position: ", profitInPips);
      //       }
      //       // If the position is short (SELL)
      //       else if (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_SELL) {
      //           // Calculate profit/loss in pips for SELL positions
      //          double profitInPips = -positionPips;
      //          if(profitInPips <= 20 && jmlTrxTerbuka == 1) {
      //             openPrice = SymbolInfoDouble(_Symbol, SYMBOL_BID);
      //             MarketOrder(ORDER_TYPE_SELL,jmlTrxTerbuka);
      ...
      //             jmlTrxTerbuka += 1 ;
      //          }
      //          Print("Profit/Loss in Pips for SELL position: ", profitInPips);
      //       }
      // }
   // }
   }
...
//--- not updated strings
   ...
   comment.SetText(8,StringFormat("Debug: %s v.%s",text),COLOR_CAPTION);
   comment.Show();
//--- run timer
   if(!tester)
      EventSetTimer(1);
